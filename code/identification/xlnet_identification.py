...
class Model(nn.Module):
    ...
    def encode(self, indexed_tokens):
        ...
        for instance in indexed_tokens:
            encoded_input = self.tokenizer.prepare_for_model(instance, max_length=max_len, pad_to_max_length=True)
        ...
        return self.encoder(input_ids=tokens_tensor, token_type_ids=token_type_ids, attention_mask=attention_mask)[0]
    ...
    def forward(self, sentences, relation):
        ...
        # retrieve, concatenate, and stack the embeddings for events
        rel_repr = []
        for para, rels in zip(sent_output, relation):
            e1, e2 = rels
            e1_idx = torch.arange(e1[0], e1[1])
            e2_idx = torch.arange(e2[0], e2[1])
            e1_repr = torch.sum(para.index_select(0, self.get_var(e1_idx)), dim=0)
            e2_repr = torch.sum(para.index_select(0, self.get_var(e2_idx)), dim=0)
...
def prepare(data, tokenizer):
    ...
    for row in data:
        ...
        relations.append([[new_idxs[ii] for ii in row['source']['idx']], [new_idxs[ii] for ii in row['target']['idx']]])
...
def train(args):
    ...
    # batchify
    batch_data = batchify(paragraphs['train'], relations['train'], labels['train'], args.batch_size)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for set_info in ['train', 'dev']:
            ...
            if set_info == 'dev':
                if f1 > dev_best:
                    print("Save Model...\n")
                    torch.save(model, os.path.join(out_dir, 'baseline_best_model.pt'))
                    best_acc = acc
